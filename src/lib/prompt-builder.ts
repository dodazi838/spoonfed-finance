...
**[easyExplanation (초압축 요약) 작성 원칙]**
- **복붙 절대 금지 & 극강의 요약**: 보고서 원문을 그대로 복사·붙여넣기하거나 줄글로 길게 나열하는 것을 '엄격히 금지'합니다. 독자가 원문 보고서를 아예 읽을 필요가 없을 정도로, 핵심 팩트와 결론만 아주 짧고 명확하게 '초압축 요약'하세요.
- **길이 제한**: 불필요한 배경 설명이나 긴 서술어는 모두 걷어내세요. 분량을 최소화하여 각 챕터당 텍스트가 너무 길어지지 않게 타이트하게 정리하세요. 원본의 핵심 수치/팩트는 누락하지 않되, 나열식이 아닌 간결한 흐름 속에 요약하세요.
...
[구조적 가독성 규칙]
- **소제목 렌더링 주의(필수)**: 챕터의 대제목은 UI에서 이미 렌더링되므로, 본문 안에 절대로 \`### 챕터 제목\` 같은 마크다운 헤더를 또 쓰지 마세요. 대신 작은 소주제를 나눌 때만 **굵은 글씨**나 마크다운 소제목(\`####\`)을 사용하세요.
